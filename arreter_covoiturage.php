...
try {
    ...
    // NOUVEAU : Vérifier si l'utilisateur connecté est le conducteur de ce covoiturage ET a le rôle de conducteur
    $utilisateur_connecte_role_id = $_SESSION['utilisateur_role_id'] ?? null;
    $ROLE_CONDUCTEUR_ID = 2; // <<< IMPORTANT : Vérifiez cette valeur dans votre DB (même valeur que dans vue_detailee_covoiturage.php)

    if ($covoiturage_info['utilisateur_id'] != $utilisateur_id_connecte || $utilisateur_connecte_role_id != $ROLE_CONDUCTEUR_ID) {
